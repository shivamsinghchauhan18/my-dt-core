...
backup_and_exclude_official_messages() {
    ...
    for msg_file in "${OFFICIAL_MESSAGES[@]}"; do
        local msg_path="$PROJECT_ROOT/packages/duckietown_msgs/msg/$msg_file"
...
verify_enhanced_namespace() {
    ...
    local enhanced_dir="$PROJECT_ROOT/packages/duckietown_msgs/msg/enhanced"
...
update_cmake_for_enhanced_messages() {
    ...
    local cmake_file="$PROJECT_ROOT/packages/duckietown_msgs/CMakeLists.txt"
...
create_safe_deployment_script() {
    ...
    cat > "$PROJECT_ROOT/scripts/deploy_safe_messages.sh" << 'EOF'
#!/bin/bash
...
EOF
    ...
    chmod +x "$PROJECT_ROOT/scripts/deploy_safe_messages.sh"
...
EOF
    ...
    log_info "🚀 Use ./scripts/deploy_safe_messages.sh for safe deployment"
}
