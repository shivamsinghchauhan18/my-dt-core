...
validate_system() {
    ...
    if [[ -f "$PROJECT_ROOT/scripts/run_system_validation.sh" ]]; then
        log_info "Running comprehensive system validation..."
        if "$PROJECT_ROOT/scripts/run_system_validation.sh" --functional-only --timeout 600; then
...
deploy_to_robot() {
    ...
    if [[ -f "$PROJECT_ROOT/scripts/enhanced_deploy.sh" ]]; then
        log_info "Using enhanced deployment script..."
        "$PROJECT_ROOT/scripts/enhanced_deploy.sh" "$ROBOT_NAME" all
    else
