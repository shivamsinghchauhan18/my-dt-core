...
sync_source_code() {
    ...
    rsync -avz --progress --delete \
        ...
        "$user@$host:$workspace/packages/" | while IFS= read -r line; do
        log_progress "  $line"
    done
    ...
    rsync -avz --progress \
        ...
        "$user@$host:$workspace/scripts/" | while IFS= read -r line; do
        log_progress "  $line"
    done
    
    # Make scripts executable
    ssh -p "$port" "$user@$host" "chmod +x $workspace/scripts/*.sh" || {
...
verify_deployment() {
    ...
    for package in "${key_packages[@]}"; do
        if ! ssh -p "$port" "$user@$host" "test -d $workspace/packages/$package" 2>/dev/null; then
...
show_usage() {
    ...
    echo
    echo "Arguments:"
    echo "  ROBOT_NAME          Name of the robot to deploy to (must have config in robot_configs/)"
    echo
    echo "Deploy Options:"
    ...
    echo "  Robot configurations should be placed in: robot_configs/ROBOT_NAME.yaml"
    echo "  See robot_configs/example.yaml for configuration format"
}
