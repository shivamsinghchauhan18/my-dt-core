...
validate_msg_compatibility() {
    ...
    if [ -d "$PROJECT_ROOT/packages/duckietown_msgs/msg/enhanced" ]; then
        local enhanced_count=$(find "$PROJECT_ROOT/packages/duckietown_msgs/msg/enhanced" -name "*.msg" | wc -l)
    ...
    local main_msg_dir="$PROJECT_ROOT/packages/duckietown_msgs/msg"
...
deploy_enhanced_vision_utils() {
    ...
    scp -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/image_processing/src/advanced_vision_utils.py" \
...
'")
    ...
    ssh_cmd "mkdir -p /tmp/backups && cp -r /code/catkin_ws/src/dt-core/packages/image_processing /tmp/backups/$backup_name"
    
    # Deploy to catkin workspace
    log_info "Deploying to catkin workspace..."
    ssh_cmd "cp /tmp/staging/advanced_vision_utils.py /code/catkin_ws/src/dt-core/packages/image_processing/src/"
...
deploy_adaptive_line_detector() {
    ...
    ssh_cmd "cp -r /code/catkin_ws/src/dt-core/packages/line_detector /tmp/line_detector_backup_$(date +%s) 2>/dev/null || true"
    
    # Backup original line detector
    ssh_cmd "cp -r /code/catkin_ws/src/dt-core/packages/line_detector /tmp/line_detector_backup_$(date +%s) 2>/dev/null || true"
    
    # Copy enhanced line detector to catkin workspace
    scp -r -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/line_detector/" \
        "$ROBOT_USER@$ROBOT_HOST:/tmp/enhanced_line_detector_staging/"
    
    # Deploy to catkin workspace after testing
    ssh_cmd "cp -r /tmp/enhanced_line_detector_staging/src/* /code/catkin_ws/src/dt-core/packages/line_detector/src/"
...
deploy_enhanced_lane_filter() {
    ...
    ssh_cmd "mkdir -p /tmp/backups && cp -r /code/catkin_ws/src/dt-core/packages/lane_filter /tmp/backups/$backup_name"
    
    # Stage enhanced lane filter for testing
    scp -r -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/lane_filter/" \
...
'"
    ...
    # Deploy to catkin workspace after testing
    ssh_cmd "cp -r /tmp/staging/enhanced_lane_filter/src/* /code/catkin_ws/src/dt-core/packages/lane_filter/src/"
...
deploy_safety_monitoring() {
    ...
    scp -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/fsm/src/safety_status_publisher.py" \
...
deploy_performance_optimizer() {
    ...
    scp -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/image_processing/src/performance_optimizer.py" \
...
deploy_enhanced_navigation() {
    ...
    ssh_cmd "mkdir -p /tmp/backups && cp -r /code/catkin_ws/src/dt-core/packages/navigation /tmp/backups/$backup_name"
    
    # Stage navigation components for testing
    scp -r -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/navigation/" \
...
'"
    ...
    # Deploy to catkin workspace after testing
    ssh_cmd "cp -r /tmp/staging/enhanced_navigation/src/* /code/catkin_ws/src/dt-core/packages/navigation/src/"
...
deploy_object_detection() {
    ...
    scp -r -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/vehicle_detection/src/" \
...
deploy_apriltag_enhancements() {
    ...
    ssh_cmd "cp -r /code/catkin_ws/src/dt-core/packages/apriltag /tmp/apriltag_backup_$(date +%s) 2>/dev/null || true"
    
    # Copy enhanced AprilTag components
    scp -r -o StrictHostKeyChecking=no "$PROJECT_ROOT/packages/apriltag/src/" \
