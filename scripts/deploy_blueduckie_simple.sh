...
ssh_cmd() {
    ...
    if command -v sshpass &> /dev/null; then
        ...
        log_info "Or set up SSH keys: ./scripts/setup_ssh_keys.sh"
        return 1
    fi
}
...
if ssh_cmd "echo 'SSH connection successful'"; then
    ...
    log_info "3. Set up SSH keys: ./scripts/setup_ssh_keys.sh"
    exit 1
fi
...
scp_cmd "robot_configs/blueduckie.yaml" "/code/catkin_ws/src/dt-core/"
