...
1. 📦 BACKUP ORIGINAL PACKAGES
   /code/catkin_ws/src/dt-core/packages/line_detector 
   → /tmp/backups/line_detector_20250828_123456

2. 🧪 STAGE & TEST IN TEMPORARY LOCATION
   Local: packages/line_detector/src/
...
3. 🚀 DEPLOY TO CATKIN WORKSPACE
   ...
   → /code/catkin_ws/src/dt-core/packages/line_detector/src/
...
deploy_enhanced_line_detector_catkin() {
    ...
    ssh_cmd "cp -r $DT_CORE_PATH/packages/line_detector /tmp/backups/$backup_name"
    ...
    scp -r packages/line_detector/ "$ROBOT_USER@$ROBOT_HOST:/tmp/staging/enhanced_line_detector/"
...
'"
    ...
    ssh_cmd "cp -r /tmp/staging/enhanced_line_detector/src/* $DT_CORE_PATH/packages/line_detector/src/"
    ...
    if ssh_cmd "pgrep -f line_detector_node.py"; then
        ...
        # Rollback
        ssh_cmd "cp -r /tmp/backups/$backup_name/* $DT_CORE_PATH/packages/line_detector/"
        ssh_cmd "cd $CATKIN_WS && source devel/setup.bash && catkin build dt-core"
        return 1
    fi
