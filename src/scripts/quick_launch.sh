...
if [ ! -f "$ROOT_DIR/src/launchers/enhanced_autonomous_system.sh" ]; then
...
MODEL_PATHS=(
    "/code/enhanced_ws/src/my-dt-core/src/packages/vehicle_detection/yolov5s.pt"
    "/code/catkin_ws/src/dt-duckiebot-interface/my-dt-core/src/packages/vehicle_detection/yolov5s.pt"
    "$ROOT_DIR/src/packages/vehicle_detection/yolov5s.pt"
)
...
if [ "$MODEL_EXISTS" = false ]; then
    ...
    if [ -f "$ROOT_DIR/src/scripts/download_yolo_model.sh" ]; then
        "$ROOT_DIR/src/scripts/download_yolo_model.sh"
    else
        # Fallback: download directly to vehicle_detection package
        TARGET_MODEL_PATH="$ROOT_DIR/src/packages/vehicle_detection/yolov5s.pt"
        ...
        if [ -f "$TARGET_MODEL_PATH" ]; then
            ...
            log_error "Failed to download YOLO model. Please place yolov5s.pt in src/packages/vehicle_detection/"
            exit 1
        fi
    fi
fi
...
EXECUTABLE_FILES=(
    "$ROOT_DIR/src/launchers/enhanced_autonomous_system.sh"
    "$ROOT_DIR/src/packages/vehicle_detection/src/enhanced_vehicle_detection_node.py"
    "$ROOT_DIR/src/packages/duckietown_demos/src/scripts/enhanced_system_startup.py"
)
...
if [ "$1" != "--skip-validation" ]; then
    ...
    if [ -f "$ROOT_DIR/src/scripts/validate_system.sh" ]; then
        if "$ROOT_DIR/src/scripts/validate_system.sh"; then
...
cd "$ROOT_DIR"
exec ./src/launchers/enhanced_autonomous_system.sh
