...
check_yolo_model() {
    ...
    if [ "$FOUND_MODEL" = false ]; then
        log_error "YOLO model not found. Run: /code/enhanced_ws/src/my-dt-core/src/scripts/download_yolo_model.sh"
        ERRORS=$((ERRORS + 1))
    fi
}
...
check_permissions() {
    ...
    EXECUTABLE_FILES=(
        "/code/enhanced_ws/src/my-dt-core/src/launchers/enhanced_autonomous_system.sh"
        "/code/enhanced_ws/src/my-dt-core/src/packages/vehicle_detection/src/enhanced_vehicle_detection_node.py"
        "/code/enhanced_ws/src/my-dt-core/src/packages/duckietown_demos/src/scripts/enhanced_system_startup.py"
    )
