...
sshpass -p "$ROBOT_PASSWORD" scp -o StrictHostKeyChecking=no \
    src/packages/line_detector/src/line_detector_node.py \
    "$ROBOT_USER@$ROBOT_HOST:/tmp/dt-core-enhanced/"

# Copy configuration
log_info "Deploying configuration..."
sshpass -p "$ROBOT_PASSWORD" scp -o StrictHostKeyChecking=no \
    src/robot_configs/pinkduckie.yaml \
...
docker run -d --name dt-lane-enhanced \\
    ...
    bash -c \"
        ...
        cp /tmp/enhanced/line_detector_node.py /code/catkin_ws/src/dt-core/src/packages/line_detector/src/
