...
detect_conflicts() {
    ...
    for package in "${enhanced_packages[@]}"; do
        if [ -d "$PROJECT_ROOT/src/packages/$package" ]; then
            ...
            if ssh_cmd "test -d $CATKIN_WS/src/dt-core/src/packages/$package"; then
                log_conflict "  → Original $package found on robot at $CATKIN_WS/src/dt-core/src/packages/$package"
                
                # Check for specific file conflicts
                if ssh_cmd "test -f $CATKIN_WS/src/dt-core/src/packages/$package/package.xml"; then
                    local original_name=$(ssh_cmd "grep '<name>' $CATKIN_WS/src/dt-core/src/packages/$package/package.xml | sed 's/.*<name>\(.*\)<\/name>.*/\1/'")
    ...
    if grep -r "node_name.*line_detector_node" "$PROJECT_ROOT/src/packages/" >/dev/null 2>&1; then
    ...
    if grep -r "segment_list" "$PROJECT_ROOT/src/packages/" >/dev/null 2>&1; then
    ...
    if grep -r "from line_detector import" "$PROJECT_ROOT/src/packages/" >/dev/null 2>&1; then
...
safe_deploy() {
    ...
    if [ -f "$PROJECT_ROOT/src/scripts/progressive_deployment.sh" ]; then
        "$PROJECT_ROOT/src/scripts/progressive_deployment.sh" --component all
    else
