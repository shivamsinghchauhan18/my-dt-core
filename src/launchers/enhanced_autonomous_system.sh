...
ensure_yolo_model() {
    ...
    local target="$ROOT_DIR/src/packages/vehicle_detection/yolov5s.pt"
    ...
    if [ -f "$target" ]; then
        ...
        log_error "Failed to obtain YOLO model. Place yolov5s.pt under src/packages/vehicle_detection/."
        exit 1
    fi
}
...
start_startup_manager() {
    ...
    STARTUP_SCRIPT="$ROOT_DIR/src/packages/duckietown_demos/src/scripts/enhanced_system_startup.py"
...
validate_system_startup() {
    if [ "$ENABLE_VALIDATION" = "true" ]; then
        ...
        VALIDATION_SCRIPT="$ROOT_DIR/src/packages/duckietown_demos/src/scripts/validate_enhanced_system.py"
